...
## A recipe for deriving guarantees based on analyzing the smallest eigenvalue of the Neural Tangent Kernel (NTK)
...
::: {.proof}
   ...
   then collecting terms we have
    ...
    \end{align*}
    Again collecting terms and substituting the expression for gradient flow given in \eqref{opt2-eq:grad-flow} we have
    \begin{align*}
...
To illustrate how the approach for proving convergence guarantees discussed in Section \ref{opt2-subsec:outline} can be applied to neural networks, here we study potentially the simplest setting possible. Consider a two layer network
\[
...
:::{.proof}
Let $i,l \in [n]$ be arbitrary and for typographical clarity let $Z_r = \phi'(\rvw^T \rvx_i)  \phi'(\rvw^T \rvx_l)$. Then by the bounded derivative assumption on $\phi$ it follows that $H_{il}(0)$ is the arithmetic average of a sum of $m$ mutually independent and identically distributed random variables, which almost surely are supported on $[-C^2, C^2]$. Applying Hoeffding's inequality
    \begin{align*}
...
:::{#lem-L0}
Suppose there exists a $C \in \reals_{>0}$ such that $\expec [\phi^2(Z)] \leq K$ and $|y_i| \leq K$ for all $i \in [n]$ and $Z \sim N(0,1)$. For $\delta \in (0,1)$, if $n \geq \delta^{-1}$ then with probability at least $1-\delta$ we have $L(0) \leq 2Cn^2$.
:::
...
However, we can circumvent this issue using a real induction argument! We now present the main result of this section.

:::{#thm-main}
Assume $\phi$ is continuously differentiable and that there exists a $C\in \reals_{> 0}$ such that $|\phi'(z)|\leq C$ for all $z \in \reals$, $\expec[\phi^2(Z)]\leq C$ with $Z \sim N(0,1)$ and $\phi'$ is $C$-Lipschitz. For $\delta \in (0,1)$, let $n \geq 2\delta^{-1}$ and assume $m \geq \max \{\frac{32C^4}{\lambda_n(\rmH_{\infty})^2} \ln \left( \frac{4n^2}{\delta} \right),  \frac{256C^6n^7 }{\lambda_n(\rmH_{\infty})^4}\}$. Then with probability at least $1-\delta$ we have for all $t \geq 0$
\begin{align*}
...
:::{.proof}
By construction $\lambda_{n}(\rmH_{\infty})\geq 0$, as the case $\lambda_{n}(\rmH_{\infty})=0$ is trivial assume $\lambda_{n}(\rmH_{\infty})>0$. To prove the result claimed recall from @lem-unifbound that it suffices to show that $\lambda_n(\rmH(t)) \geq \lambda_n(\rmH_{\infty})/2$ for all $t >0$. From @lem-shadowmatrix for this condition to be true it suffices that $||\rmH(t) - \rmH(0)||,  ||\rmH(0) - \rmH_{\infty}|| \leq \lambda_n(\rmH_{\infty})/4$ for all $t \geq 0$. Note as 
$$m \geq \frac{32C^4}{\lambda_n(\rmH_{\infty})^2} \ln \left( \frac{4n^2}{\delta} \right)
$$
...
For Statement iii), if $[0,t) \subset S$ then for any $\delta'>0$ it follows that $[0, t - \delta'] \subset S$. Using @lem-smoothDynamics
    ...
    \end{align*}
  Letting $\delta \leq \frac{1}{2\lambda_n(\rmH_{\infty})}$, then as $m\geq \frac{256C^6n^7 }{\lambda_n(\rmH_{\infty})^4}$ it follows that
    $$
